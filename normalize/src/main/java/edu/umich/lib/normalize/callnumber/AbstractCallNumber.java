 ...
 *
 * <p>Provides tests to see if the given callnumber is considered valid in the implementing schema,
 * and allows for truncated values (which aren't valid but still can be useful, esp. for range
 * queries) creating "invalid" keys, which produce a potentially-useful key despite the fact that
 * the underlying callnumber isn't valid as defined by the implementation.
 *
 * @author Bill Dueber
 */
public abstract class AbstractCallNumber {
  ...
  /**
   * The original string with schema-aware non-semantic leading/trailing characters (e.g., spaces)
   * removed
   */
  ...
  /**
   * Some schemes have "truncated" versions that can be useful for sorting/searching (e.g., an LC
   * call number requires both an alphabetic and numeric portion, but it can be useful to accept
  ...
  /**
   * Returns the "invalid" key — a transformed version of the input string that can still be used
   * for sorting/searching even when the call number does not conform to the implementing scheme.
   *
  ...
  /**
   * Returns the best available key for this call number, preferring validity over truncation, and
   * echoing the invalid key as a last resort.
   *
   ...
   *
   * <ol>
   *   <li>Returns {@link #validKey()} if {@link #hasValidKey()} is {@code true}.
   *   <li>Returns {@link #acceptableTruncatedKey()} if {@code allowTruncated} is {@code true} and
   *       {@link #hasAcceptableTruncatedKey()} is {@code true}.
   *   <li>Returns {@link #invalidKey()} if {@code echoInvalidInput} is {@code true}.
   *   <li>Returns {@code null} if none of the above conditions are met.
   * </ol>
   *
   * @param allowTruncated when {@code true}, an acceptable truncated key may be returned if no
   *     valid key is available
   * @param echoInvalidInput when {@code true}, the processed invalid key is returned as a final
   *     fallback
  ...
  public static Pattern TRIM_PUNCT = Pattern.compile("^\\p{Punct}*(.*?)\\p{Punct}*$");

  /**
   * Removes leading and trailing punctuation characters from {@code str}.
   *
   * @param str the string to process
   * @return {@code str} with leading and trailing punctuation stripped, or the original string
   *     unchanged if it contains no punctuation boundary
   */
  ...
  /**
   * Trims leading/trailing whitespace and collapses any run of internal whitespace characters to a
   * single space.
   *
   * @param str the string to process
