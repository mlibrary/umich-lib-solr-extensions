 ...
 *
 * <p>Always uses the proper lifecycle: {@code reset()} before consuming, {@code end()} + {@code
 * close()} after. This mirrors what Solr does internally and catches resource-leak or
 * improper-reset bugs.
 */
public final class TokenStreamAsserter {
   ...
   *
   * <p>The caller is responsible for closing the stream (or the enclosing {@link Analyzer}) after
   * this call.
   *
  ...
  /**
   * Runs the given {@code text} through the {@code analyzer} on a synthetic field named {@code
   * "field"} and returns the resulting tokens.
   */
  ...
  /**
   * Asserts that the token stream produces exactly the expected term strings, in order. Position
   * increments are not checked by this overload.
   */
  public static void assertTerms(List<TokenData> actual, String... expectedTerms) {
    ...
    assertEquals(
        ...
        actualTerms,
        String.format("Expected terms %s but got %s", Arrays.toString(expectedTerms), actualTerms));
  }

  /**
   * Asserts that each token's position increment matches the expected value. The lists must be the
   * same length; use {@link #assertTerms} first to confirm token count.
   */
  public static void assertPositionIncrements(List<TokenData> actual, int... expectedIncrements) {
    assertEquals(
        expectedIncrements.length, actual.size(), "Token count mismatch — check assertTerms first");
    for (int i = 0; i < expectedIncrements.length; i++) {
      ...
      assertEquals(
          ...
          String.format(
              "Position increment mismatch at token %d ('%s')", idx, actual.get(idx).term()));
    }
  }
