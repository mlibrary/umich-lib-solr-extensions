...
/**
 * Tests for {@link UpcaseWordsThatStartWithFilter} and {@link
 * UpcaseWordsThatStartWithFilterFactory}.
 */
...
class UpcaseWordsThatStartWithFilterTest {
  ...
  class TokenUppercasing {
    ...
    void mixedStream_selectiveUppercase() throws IOException {
      try (var a = analyzer(Map.of("letter", "a"))) {
        assertTerms(
            analyze(a, "apple banana avocado cherry"), "APPLE", "banana", "AVOCADO", "cherry");
      }
    }

    @Test
    @DisplayName(
        "README example: 'apples and bananas' with letter=a, reverse=false → [APPLES, AND, bananas]")
    ...
    @Test
    @DisplayName(
        "README example: 'apples and bananas' with letter=a, reverse=true → [SELPPA, DNA, bananas]")
