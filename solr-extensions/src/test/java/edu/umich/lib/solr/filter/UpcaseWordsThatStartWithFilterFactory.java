 ...
 *
 * <p>Required parameter: {@code letter} — a single alphabetic character. Optional parameter: {@code
 * reverse} (boolean, default {@code false}).
 *
 * <p>The {@code echoInvalidInput} parameter has no practical effect because {@link
 * UpcaseWordsThatStartWithFilter#munge(String)} never returns {@code null}.
 *
  ...
  public UpcaseWordsThatStartWithFilterFactory(Map<String, String> args) {
    ...
    if (raw == null) {
      throw new IllegalArgumentException(
          "UpcaseWordsThatStartWithFilterFactory requires a 'letter' argument");
    }
    if (raw.length() != 1) {
      throw new IllegalArgumentException(
          "UpcaseWordsThatStartWithFilterFactory 'letter' must be exactly one character; got: \""
              + raw
              + "\"");
    }
    if (!Character.isLetter(raw.charAt(0))) {
      throw new IllegalArgumentException(
          "UpcaseWordsThatStartWithFilterFactory 'letter' must be an alphabetic letter; got: \""
              + raw
              + "\"");
    }
  }
