 ...
 *
 * <p>{@code toInternal()} and {@code bundledFields()} are pure logic that does not require a live
 * Solr container. We instantiate the field type directly and rely on its default configuration
 * ({@code allowTruncated=true}, {@code echoInvalidInput=false}).
 */
...
class CallNumberSortKeyFieldTypeTest {
  ...
  class TruncatedAndInvalidInputs {
    ...
    void noTruncation() {
      ...
      assertNull(
          strict.toInternal("PS"), "allowTruncated=false should reject truncated-only input");
    }
  }
}
