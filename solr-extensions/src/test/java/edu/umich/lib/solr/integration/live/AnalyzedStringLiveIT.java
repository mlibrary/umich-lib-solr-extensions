 ...
 *
 * <p>AnalyzedString extends StrField: values fed to the field are run through the index analysis
 * chain of a referenced TextField and stored as a plain string. This guarantees that range queries
 * compare pre-normalized values.
 */
...
class AnalyzedStringLiveIT extends AbstractLiveIT {
  ...
  /**
   * Values stored in an AnalyzedString field are normalized at index time: uppercase → lowercase,
  ...
  void rangeQueryMatchesNormalizedValues() throws Exception {
    ...
    QueryResponse resp =
        client.query(new SolrQuery("author_browse:[bjork TO \"rolling stones\"]").setRows(10));
  ...
  void openLowerBoundRange() throws Exception {
    ...
    QueryResponse resp = client.query(new SolrQuery("author_browse:[* TO beatles]").setRows(10));
    List<String> ids = idsOf(resp);
  ...
  void openUpperBoundRange() throws Exception {
    ...
    QueryResponse resp =
        client.query(new SolrQuery("author_browse:[\"rolling stones\" TO *]").setRows(10));
    List<String> ids = idsOf(resp);
  ...
  private List<String> idsOf(QueryResponse resp) {
    ...
    return results.stream().map(d -> (String) d.getFieldValue("id")).collect(Collectors.toList());
  }
}
