...
/**
 * A Solr/Lucene token filter that normalizes LC or Dewey call number tokens to a sortable key
 * string suitable for sort fields and (when combined with an edge-ngram filter) left-anchored
 * prefix search.
 *
 * <p>Tokens that cannot be parsed as a call number return {@code null} from {@link #munge(String)},
 * which causes {@link SimpleFilter} to either drop the token or echo it unchanged, depending on the
 * {@code echoInvalidInput} setting.
 *
...
public final class AnyCallNumberNormalizerFilter extends SimpleFilter {
   ...
   */
  public AnyCallNumberNormalizerFilter(
