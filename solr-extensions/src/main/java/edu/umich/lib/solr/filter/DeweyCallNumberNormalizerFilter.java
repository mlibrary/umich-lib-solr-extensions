...
/**
 * A Solr/Lucene token filter that normalizes Dewey call number tokens to a sortable key string
 * suitable for both sort fields and left-anchored prefix search (when paired with an edge-ngram
 * filter).
 *
 * <p>Tokens that cannot be parsed as a Dewey call number return {@code null} from {@link
 * #munge(String)}, which causes {@link SimpleFilter} to either drop the token or echo it unchanged,
 * depending on the {@code echoInvalidInput} setting.
 *
...
public final class DeweyCallNumberNormalizerFilter extends SimpleFilter {
   ...
   * @param echoInvalidInput when {@code true}, tokens that cannot be parsed are passed through
   *     unchanged
   * @param args all schema.xml attributes forwarded from the factory
   */
  public DeweyCallNumberNormalizerFilter(
   ...
   */
  public DeweyCallNumberNormalizerFilter(
      TokenStream in, boolean allowTruncated, boolean echoInvalidInput) {
    super(in, echoInvalidInput, Map.of());
