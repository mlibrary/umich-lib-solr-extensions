 ...
 *
 * <p>{@link LCCNNormalizer#normalize(String)} handles all inputs gracefully; it never throws and
 * returns the input unchanged when no LCCN pattern is found. Consequently this filter always passes
 * tokens through (normalized or as-is) and never drops them.
 *
...
public final class LCCNNormalizerFilter extends SimpleFilter {
   ...
   * @param echoInvalidInput forwarded to {@link SimpleFilter}; effectively unused because {@code
  ...
  /**
   * Normalizes {@code input} as an LCCN. Always returns a non-null string; inputs that are not
   * LCCNs are returned unchanged.
   */
  @Override
