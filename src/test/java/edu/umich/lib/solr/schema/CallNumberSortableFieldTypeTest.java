 ...
 * <em>original</em> display value while inheriting sort-key normalisation from
 * {@link CallNumberSortKeyFieldType}.
 *
 * <p>The {@code createField()} override requires a live {@code SchemaField} (which in
 * turn needs an {@code IndexSchema}); that path is covered by
 * {@link edu.umich.lib.solr.integration.live.CallNumberSortLiveIT}.  These unit tests
 * focus on the behaviours that are exercisable without a Solr container:
 * <ul>
 *   <li>Inheritance: {@code toInternal()} works identically to the parent.</li>
...
class CallNumberSortableFieldTypeTest {
    ...
    class InheritedToInternal {
        ...
        void sortOrderPreserved() {
            ...
            assertNotNull(a); assertNotNull(b); assertNotNull(c); assertNotNull(d);
    ...
    class GateLogic {
        ...
        @Test
        @DisplayName("truncated LC (letters only): hasAcceptableTruncatedKey=true → field will be created when allowTruncated=true")
        void truncatedLCHasTruncatedKey() {
            ...
            assertTrue(cn.hasAcceptableTruncatedKey(),
        ...
        void noTruncationSkipsField() {
            ...
            assertNull(fieldType.toInternal("PS"),
                "allowTruncated=false should reject truncated-only LC stub");
        }
    }
}
