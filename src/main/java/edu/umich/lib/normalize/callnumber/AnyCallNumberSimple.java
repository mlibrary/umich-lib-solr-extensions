...
/**
 * A call number that accepts either LC or Dewey format, picking the first
 * valid interpretation.  If neither format produces a valid parse, the
 * input is treated as an unrecognised string and returned via
 * {@link #invalidKey()}.
 */
public class AnyCallNumberSimple extends AbstractCallNumber {
  ...
  /**
   * Parses {@code str} as both an LC and a Dewey call number and selects
   * whichever format produces a valid key (LC takes precedence).
   *
  ...
  /**
   * Returns the truncated key preferred in LC order: LC key first, then
   * Dewey, then {@code null} if neither applies.
   */
  @Override
  ...
  /**
   * Returns a freetext fallback key for unrecognised call numbers by
   * delegating to the LC parser's {@code invalidKey()} implementation.
   */
  @Override
