 ...
 *
 * <p>All public methods return {@code null} when the input does not contain a
 * recognizable ISBN, rather than throwing.  A DEBUG-level log message is
 * emitted for diagnostic purposes when extraction fails.
 *
...
public final class ISBNNormalizer {
    ...
    /**
     * Try to extract an ISBN from the string.  ISBN-13s are returned as-is;
     * ISBN-10s are converted to ISBN-13 and returned.
     *
    ...
    /**
     * Extracts an ISBN from {@code isbnstring} using the given pattern and
    ...
    public static String extractIsbnByPat(String isbnstring, Pattern pat, int len) {
        ...
        if (normalized.length() != len) {
            LOGGER.debug("'{}' matched ISBN{} pattern but has wrong length: {}", normalized, len, normalized.length());
            return null;
        }
        return normalized;
    }
